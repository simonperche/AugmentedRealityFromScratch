...
namespace arfs
{
    // Define unnamed namespace for private usage
    namespace
    {
        /**
         * @brief Augment a list of objects using camera
         * @param frame
         * @param objects
         * @param camera
         */
        void augmentObjects(const cv::Mat& frame, const std::vector<arfs::Object>& objects, const arfs::Camera& camera);

        /**
         * @brief Apply a projection matrix on 3D points to get image points.
         * @param points3D points
         * @param projectionMatrix
         * @return array of image points
         */
        std::vector<cv::Point2i> projectPoint(const std::vector<cv::Point3d>& points, const cv::Mat& projectionMatrix);
    }
    ...
    namespace
    {
        void augmentObjects(const cv::Mat& frame, const std::vector<arfs::Object>& objects, const arfs::Camera& camera)
        {
            ...
            for(auto& face : faces)
            {
                auto scenePoints = projectPoint(face.points, camera.getProjectionMatrix());

                //Flat shading
                auto light = cv::Vec3d(1, 0, 0);
                auto angle = arfs::Utils::Geometry::angleBetween(face.normal, light, arfs::AngleType::DEG);
                auto lightValue = angle / 180;

                cv::fillConvexPoly(frame, scenePoints, face.color*lightValue);
            }
        }

        std::vector<cv::Point2i> projectPoint(const std::vector<cv::Point3d>& points, const cv::Mat& projectionMatrix)
        {
            std::vector<cv::Point2i> scenePoints(points.size());
            for(int i = 0; i < points.size(); i++)
            {
                ...
                scenePoints[i].x = int(tmp.at<double>(0, 0));
                scenePoints[i].y = int(tmp.at<double>(1, 0));
            }
